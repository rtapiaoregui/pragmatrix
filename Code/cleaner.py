...
from docs_normalizer import docs_normalizer as norm
...
# Short fiction:
def eeuu_cleaner(cleaner_paths):
    
    os.chdir(cleaner_paths['eeuu_path'])
    dirty = []
    for i in os.listdir():
        file = open(i, 'r')
        s = file.read()
...
def short_fic_cleaner(cleaner_paths, specific_path):
 
    os.chdir(cleaner_paths.get(specific_path))
...
def short_fic_cleaner2(cleaner_paths, specific_path):
...
def arab_cleaner(cleaner_paths):
    
    os.chdir(cleaner_paths['arab_path'])
...
# Long fiction
def adelaide_cleaner(cleaner_paths):
    
    os.chdir(cleaner_paths['adelaide_path'])
    ...
    for piece in dirty:
        ...
        if cond_eng1 and cond_eng2 and cond_eng3 and cond_eng4 and cond_eng5 and cond_junk0 and cond_junk1:
...
# NOT LITERATURE
def ling_blogs_cleaner(cleaner_paths, specific_path):
...
def wiki_cleaner(cleaner_paths):
    
    os.chdir(cleaner_paths['wiki_path'])
...
# News:
def news_cleaner(cleaner_paths, specific_path):
...
# Short fiction:
def resource_main(cleaner_paths):
    resource_corpora = short_fic_cleaner(cleaner_paths, 'resource_path')
...
def electric_main(cleaner_paths):
    electric_corpora = short_fic_cleaner(cleaner_paths, 'electric_path')
...
def usa_main(cleaner_paths):
    usa_corpora = short_fic_cleaner2(cleaner_paths, 'usa_path')
    usa_corpora = [norm(i) for i in usa_corpora if not norm(i) == None]
    usa_corpora = sum(usa_corpora, [])
    return usa_corpora


def ny_fiction_main(cleaner_paths):
    ny_fiction_corpora = short_fic_cleaner2(cleaner_paths, 'ny_fiction_path')
...
def eeuu_main(cleaner_paths):
    eeuu_corpora = eeuu_cleaner(cleaner_paths)
    eeuu_corpora = [norm(i) for i in eeuu_corpora if not norm(i) == None]
    eeuu_corpora = sum(eeuu_corpora, [])
    return eeuu_corpora


def arab_st_main(cleaner_paths):
    arab_corpora = arab_cleaner(cleaner_paths)
...
# Long fiction:
def adelaide_main(cleaner_paths):
    adelaide_corpus = adelaide_cleaner(cleaner_paths)
...
# Linguistic blogs:
def oxford_main(cleaner_paths):
    oxford_corpora = ling_blogs_cleaner(cleaner_paths, 'oxford_path')
    oxford_corpora = [norm(i) for i in oxford_corpora if not norm(i) == None]
    oxford_corpora = sum(oxford_corpora, [])
    return oxford_corpora


def collins_main(cleaner_paths):
    collins_corpora = ling_blogs_cleaner(cleaner_paths, 'collins_path')
...
# Wikipedia:
def wiki_main(cleaner_paths):
    wiki_corpora = wiki_cleaner(cleaner_paths)
...
# News:
def nytimes_main(cleaner_paths):
    nytimes_corpora = news_cleaner(cleaner_paths, 'nytimes_path')
...
def washington_main(cleaner_paths):
    washington_corpora = news_cleaner(cleaner_paths, 'washington_path')
...
def independent_main(cleaner_paths):
    independent_corpora = news_cleaner(cleaner_paths, 'independent_path')
...
def bbc_main(cleaner_paths):
    bbc_corpora = news_cleaner(cleaner_paths, 'bbc_path')
    bbc_corpora = [norm(i) for i in bbc_corpora if not norm(i) == None]
    bbc_corpora = sum(bbc_corpora, [])
    return bbc_corpora


def guardian_main(cleaner_paths):
    guardian_corpora = news_cleaner(cleaner_paths, 'guardian_path')
...
def latimes_main(cleaner_paths):
    latimes_corpora = news_cleaner(cleaner_paths, 'latimes_path')
...
def daily_main(cleaner_paths):
    daily_corpora = news_cleaner(cleaner_paths, 'daily_path')
    daily_corpora = [norm(i) for i in daily_corpora if not norm(i) == None]
    daily_corpora = sum(daily_corpora, [])
    return daily_corpora


def sfchronicle_main(cleaner_paths):
    sfchronicle_corpora = news_cleaner(cleaner_paths, 'sfchronicle_path')
...
def houston_main(cleaner_paths):
    houston_corpora = news_cleaner(cleaner_paths, 'houston_path')
...
def india_main(cleaner_paths):
    india_corpora = news_cleaner(cleaner_paths, 'india_path')
...
def dataframer(cleaner_paths, paths):

    # Literature
    arab_corpora = arab_st_main(cleaner_paths)
    eeuu_corpora = eeuu_main(cleaner_paths)
    resource_corpora = resource_main(cleaner_paths)
    electric_corpora = electric_main(cleaner_paths)
    usa_corpora = usa_main(cleaner_paths)
    ny_fiction_corpora = ny_fiction_main(cleaner_paths)
    
    adelaide_corpora = adelaide_main(cleaner_paths)
    ...
    # Not literature
    oxford_corpora = oxford_main(cleaner_paths)
    collins_corpora = collins_main(cleaner_paths)
    
    wiki_corpora = wiki_main(cleaner_paths)
    
    nytimes_corpora = nytimes_main(cleaner_paths)
    washington_corpora = washington_main(cleaner_paths)
    independent_corpora = independent_main(cleaner_paths)
    bbc_corpora = bbc_main(cleaner_paths)
    guardian_corpora = guardian_main(cleaner_paths)
    latimes_corpora = latimes_main(cleaner_paths)
    daily_corpora = daily_main(cleaner_paths)
    sfchronicle_corpora = sfchronicle_main(cleaner_paths)
    india_corpora = india_main(cleaner_paths)
    houston_corpora = houston_main(cleaner_paths)
